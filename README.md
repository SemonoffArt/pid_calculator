...
Приложение определяет **FOPDT-модель** (First Order Plus Dead Time) или **IPDT-модель** (Integrating Process with Dead Time — интегрирующее звено с запаздыванием) процесса и рассчитывает оптимальные параметры регулятора **11 методами** настройки, симулирует замкнутую систему и позволяет сравнить результаты всех методов в одном месте.
...
- **Идентификация IPDT-модели** — интегрирующее звено с запаздыванием по переходной характеристике (наклон разгона + запаздывание)
- **11 методов настройки PID:**
  ...
  - AMIGO
  - ITAE-оптимизация
  - *Для IPDT* — Зиглер-Николс (замкнутый), IMC, SIMC, ITAE (PI-настройка)
...
Все фронтенд-библиотеки подключены локально из `static/vendor/` — приложение полностью работает без доступа к интернету (офлайн).
