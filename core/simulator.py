...
Регулятор в ISA-стандартной (параллельной) форме с фильтром на D:
    u = Kp*(e + 1/Ti*∫e dt + Td*d(PV)/dt/(1 + Td/N·s))
Производная берётся от ИЗМЕРЕНИЯ (PV), а не от ошибки — это исключает
«derivative kick» при ступеньке задания (иначе выход уходит в упор).
Сигнал D пропускается через звено первого порядка с постоянной времени
Td/N (N = d_filter_n). Объект интегрируется ТОЧНО
...
def _simulate(K: float, T: float, tau: float,
    ...
    y_prev = y                              # предыдущее PV (для D-составляющей)
    ...
    for k in range(total):
        ...
        dterm = 0.0
        if td and td > 0:
            # D-составляющая по ИЗМЕРЕНИЮ (PV), а не по ошибке. Это стандартная
            # ISA-практика, устраняющая «derivative kick» при ступеньке задания:
            # если брать d(ошибки)/dt, то при скачке SP возникает огромный
            # выброс выхода (CV уходит в упор). Здесь D = -Td*d(PV)/dt
            # (фильтр Td/N), т.е. противодействует изменению PV и не реагирует
            # на сам момент ступеньки SP.
            d_raw = -(y - y_prev) / h
            # Точная (ZOH) дискретизация фильтра D-составляющей с постоянной
            # времени Td/N. Метод Эйлера неустойчив/осциллирует, если шаг h
            # сравним с Td/N (при агрессивных настройках Td мало) — отсюда
            # ложные колебания выхода в упор.
            tau_f = td / d_filter_n
            a_f = float(np.exp(-h / tau_f))
            dfilt = a_f * dfilt + (1.0 - a_f) * d_raw
            dterm = td * dfilt
        y_prev = y
